...
process, where a programming language can extends, is called a
/macro/. A common use of macros is in configuration files, where we
need a high level way of defining how a program should run.
...
run when initializing the package (defined after the =:init= keyword,
not shown), and configuring the package (defined after the =:config=
keyword).
...
avoid separating configuration from code.
...
application across a variety of domain specific languages.
...
using javascript in browser extensions, what allows data scientists to
iterate on their machine learning models in Python without having to
restart, or recompile, their programs over and over again.
